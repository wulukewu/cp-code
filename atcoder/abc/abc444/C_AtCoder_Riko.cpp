 ...
 * Platform:       AtCoder
 * Problem:        abc444_c — AtCoder Riko
 * Problem URL:    https://atcoder.jp/contests/abc444/tasks/abc444_c
 *
 * Matched result: AC
 * Submission:     #73078621
 * Submission URL: https://atcoder.jp/contests/abc444/submissions/73078621
 * Submitted:      2026-02-07 20:40:14 +0800
 * Language:       C++23 (GCC 15.2.0)
 * Runtime:        33 ms
 * Points:         350.0
 * Submissions:    4 total / 1 accepted
 * File commit:    2026-02-07 21:41:56 +0800
...
void solve(){
    ...
    int l1 = arr[0] + arr[n-1];
    bool d1 = true;
    FOR(i, 0, n/2){
        if(arr[i]+arr[n-1-i]!=l1) d1 = false;
    }
    if(n%2==1) d1 = false;
    if(d1) ans.insert(l1);
    ...
    if(m%2==1) d3 = false;
    FOR(i, 0, m/2){
        if(arr[i]+arr[m-1-i]!=l3) d3 = false;
    }
    if(d3) ans.insert(l3);

    // int l4 = arr[0];
    // bool d4 = true;
    // if(arr[0]<arr[m-1]) d4 = false;
    // if(d4) ans.insert(l4);

    // map<int,int>mp;
    // for(int i: arr) mp[i]++;
    // vector<pair<int,int>>brr;
    // for(auto i: mp) brr.PB(i);
    // bool d3 = false;
    // int l3;
    // if(mp.size()==2){
    //     l3 = brr[0].F*2;
    //     if(brr[0].F*2==brr[1].F and brr[0].S%2==0){
    //         d3 = true;
    //     }
    // }else if(mp.size()==3){
    //     l3 = brr[0].F+brr[1].F;
    //     if(brr[0].F+brr[1].F==brr[2].F and brr[0].S==brr[1].S){
    //         d3 = true;
    //     }
    // }
    // if(d3) ans.insert(l3);
