 ...
 * Platform:       AtCoder
 * Problem:        abc448_e — Simple Division
 * Problem URL:    https://atcoder.jp/contests/abc448/tasks/abc448_e
 *
 * Matched result: AC
 * Submission:     #73928725
 * Submission URL: https://atcoder.jp/contests/abc448/submissions/73928725
 * Submitted:      2026-03-07 21:23:26 +0800
...
void solve(){
    ...
    int k, m;
    cin >> k >> m;
    vector<pair<int, int>>cl(k);
    FOR(i, 0, k){
        cin >> cl[i].F >> cl[i].S;
    }

    auto power = [](int base, int exp, int mod) -> int {
        int res = 1;
        base %= mod;
        while(exp){
            if(exp%2==1){
    ...
    auto cal = [&](int mod) -> int {
        int res = 0;
        for(auto& b: cl){
            int c = b.F;
            int l = b.S;

            int pm = power(10, l, mod);
            int p9m = power(10, l, 9*mod);
            int term = (p9m-1)/9;
            int add = (c*term)%mod;
            res = (res*pm%mod + add) % mod;
        }
        return res;
    };
