 ...
 * Platform:       AtCoder
 * Problem:        abc450_c — Puddles
 * Problem URL:    https://atcoder.jp/contests/abc450/tasks/abc450_c
 *
 * Matched result: AC
 * Submission:     #74294464
 * Submission URL: https://atcoder.jp/contests/abc450/submissions/74294464
 * Submitted:      2026-03-21 20:41:42 +0800
...
void solve(){
    ...
    FOR(i, 0, h){
        FOR(j, 0, w){
            int d = visit[i][j];
            if(ids.find(d)==ids.end()) continue;
            FOR(k, 0, 4){
                int r = i + offs[k][0];
                int c = j + offs[k][1];
                if(r==-1 or r==h or c==-1 or c==w or (arr[r][c]=='.' and visit[r][c]!=d)){
                    ids.erase(d);
                }
            }
        }
