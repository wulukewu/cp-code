...
........:::.......:::..::::..::........::
 ...
 ATCODER // abc450_e // Fibonacci String
 https://atcoder.jp/contests/abc450/tasks/abc450_e

 AC // C++23 (GCC 15.2.0) // 68 ms // 450.0 pt
 #74308509 // 2026-03-21 21:18:17 +0800
 https://atcoder.jp/contests/abc450/submissions/74308509
...
void solve(){
    ...
    int l1 = x.size();
    int l2 = y.size();
    vector<vector<int>>prex(l1+1, vector<int>(26, 0));
    vector<vector<int>>prey(l2+1, vector<int>(26, 0));
    FOR(i, 0, l1){
        FOR(c, 0, 26){
            prex[i+1][c] = prex[i][c];
        }
    ...
    int sz = 2;
    while(frr[sz-1]<2e18){
        frr.PB(frr[sz-1]+frr[sz-2]);
        vector<int>cur(26, 0);
        FOR(c, 0, 26){
            cur[c] = cnt[sz-1][c] + cnt[sz-2][c];
        }
        cnt.PB(cur);
        sz++;
    }
    ...
    // in sk, [0, p], char c, times
    auto cal = [&](auto&& self, int k, int p, int c) -> int {
        if(p==0) return 0;
        if(k==0) return prex[p][c];
        if(k==1) return prey[p][c];

        if(p==frr[k]) return cnt[k][c];
        if(p<=frr[k-1]) return self(self, k-1, p, c);

        return cnt[k-1][c] + self(self, k-2, p-frr[k-1], c);
    };
    ...
    while(q--){
        ...
        cin >> l >> r >> c;
        int cc = c-'a';
        int a = cal(cal, m, r, cc);
        int b = cal(cal, m, l-1, cc);
        int ans = a - b;
        // cout << b << ' ' << a << ' ' << ans << endl;
        cout << ans << endl;
    }
