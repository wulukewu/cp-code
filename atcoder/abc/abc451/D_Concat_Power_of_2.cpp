 ...
 * Platform:       AtCoder
 * Problem:        abc451_d — Concat Power of 2
 * Problem URL:    https://atcoder.jp/contests/abc451/tasks/abc451_d
 *
 * Matched result: AC
 * Submission:     #74493620
 * Submission URL: https://atcoder.jp/contests/abc451/submissions/74493620
 * Submitted:      2026-03-28 20:54:11 +0800
...
void solve(){
    ...
    while(!pq.empty()){
        ...
        for(auto& p: v){
            ...
            // if(sx.size()+si.size()<=10){
            //     int a = stoll(sx+si);
            //     if(a<=m and st.find(a)==st.end()){
            //         st.insert(a);
            //         pq.push(a);
            //     }
            //     int b = stoll(si+sx);
            //     if(b<=m and st.find(b)==st.end()){
            //         st.insert(b);
            //         pq.push(b);
            //     }
            // }

            // int asz = to_string(i).size();
            // int bsz = to_string(x).size();
            // int a = x * pow(10,asz) + i;
            // int b = i * pow(10,bsz) + x;
            // // cout << asz << ' ' << bsz << endl;
            // // cout << x << ' ' << a << ' ' << b << endl;
            // if(st.find(a)==st.end()){
            //     st.insert(a);
