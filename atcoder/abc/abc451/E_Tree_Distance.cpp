 ...
 * Platform:       AtCoder
 * Problem:        abc451_e — Tree Distance
 * Problem URL:    https://atcoder.jp/contests/abc451/tasks/abc451_e
 *
 * Matched result: AC
 * Submission:     #74508212
 * Submission URL: https://atcoder.jp/contests/abc451/submissions/74508212
 * Submitted:      2026-03-28 21:40:12 +0800
...
void solve(){
    ...
    FOR(i, 1, n+1){
        int u = -1;
        FOR(j, 1, n+1){
            if(!visit[j] and (u==-1 or me[j]<me[u])) u = j;
        }
        visit[u] = true;

        if(parent[u]!=-1){
            adj[u].PB({parent[u], arr[parent[u]][u]});
            adj[parent[u]].PB({u, arr[parent[u]][u]});
        }
        
        FOR(v, 1, n+1){
            if(!visit[v] and arr[u][v]<me[v]){
                me[v] = arr[u][v];
                parent[v] = u;
            }
        }
