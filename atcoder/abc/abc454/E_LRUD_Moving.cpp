 ...
 * Platform:       AtCoder
 * Problem:        abc454_e — LRUD Moving
 * Problem URL:    https://atcoder.jp/contests/abc454/tasks/abc454_e
 *
 * Matched result: AC
 * Submission:     #75084054
 * Submission URL: https://atcoder.jp/contests/abc454/submissions/75084054
 * Submitted:      2026-04-18 21:24:34 +0800
 * Language:       C++23 (GCC 15.2.0)
 * Runtime:        168 ms
 * Points:         450.0
 * Submissions:    1 total / 1 accepted
 * File commit:    2026-04-18 21:33:50 +0800
...
void solve(){
    ...
    while(r2-r1>1 or c2-c1>1){
        if(r2-r1>1){
            if(a>r1+1){
                // top
                head += string(c2-c1, 'R') + "D" + string(c2-c1, 'L') + "D";
                r1 += 2;
            }else{
                // bottom
                string tmp = "D" + string(c2-c1, 'L') + "D" + string(c2-c1, 'R');
                tail = tmp + tail;
                r2 -= 2;
            }
        }else{
            if(b>c1+1){
                // left
                head += string(r2-r1, 'D') + "R" + string(r2-r1, 'U') + "R";
                c1 += 2;
            }else{
                // right
                string tmp = "R" + string(r2-r1, 'U') + "R" + string(r2-r1, 'D');
                tail = tmp + tail;
                c2 -= 2;
            }
        }
