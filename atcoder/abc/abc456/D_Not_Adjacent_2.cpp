 ...
 * Platform:       AtCoder
 * Problem:        abc456_d — Not Adjacent 2
 * Problem URL:    https://atcoder.jp/contests/abc456/tasks/abc456_d
 *
 * Matched result: AC
 * Submission:     #75433429
 * Submission URL: https://atcoder.jp/contests/abc456/submissions/75433429
 * Submitted:      2026-05-02 20:27:36 +0800
...
void solve(){
    ...
    FOR(i, 0, n){
        if(s[i]=='a'){
            dp[0][i+1] = dp[0][i] + dp[1][i] + dp[2][i] + 1;
            dp[1][i+1] = dp[1][i];
            dp[2][i+1] = dp[2][i];
        }else if(s[i]=='b'){
            dp[0][i+1] = dp[0][i];
            dp[1][i+1] = dp[0][i] + dp[1][i] + dp[2][i] + 1;
            dp[2][i+1] = dp[2][i];
        }else if(s[i]=='c'){
            dp[0][i+1] = dp[0][i];
            dp[1][i+1] = dp[1][i];
            dp[2][i+1] = dp[0][i] + dp[1][i] + dp[2][i] + 1;
        }
        FOR(j, 0, 3){
            dp[j][i+1] %= M;
        }
