...
........:::.......:::..::::..::........::
 ...
 ATCODER // abc456_f // Plan Holidays
 https://atcoder.jp/contests/abc456/tasks/abc456_f

 TLE // C++23 (GCC 15.2.0) // 0.0 pt
 #75458520 // 2026-05-02 21:35:44 +0800
 https://atcoder.jp/contests/abc456/submissions/75458520
    ...
    for(int w=k; w<=min(n, k+1); w++){
        for(int l=0; l+w-1<n; l++){
            ...
            if(w==1){
                ...
                dp[1] = arr[l+1] + dp[0];
                FOR(i, 2, w){
                    dp[i] = arr[l+i] + min(dp[i-1], dp[i-2]);
                }
                ans = min(ans, dp[w-1]);
            }
        }
