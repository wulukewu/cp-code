...
void solve(){
    ...
    // int dp[m][3][2][1024];
    vector<vector<vector<vector<int>>>> dp(m, vector<vector<vector<int>>>(3, vector<vector<int>>(2, vector<int>(1024, -1))));

    auto dfs = [&](auto&& self, int pos, bool is_limit, bool is_started, int mod3, bool has3, int mask) -> int {
        if(pos==m){
    ...
    int res = dfs(dfs, 0, true, false, 0, false, 0);
    cout << res << endl;
}
