 ...
 * Platform:       AtCoder
 * Problem:        abc466_e — Range Flip
 * Problem URL:    https://atcoder.jp/contests/abc466/tasks/abc466_e
 *
 * Matched result: AC
 * Submission:     #77401442
 * Submission URL: https://atcoder.jp/contests/abc466/submissions/77401442
 * Submitted:      2026-07-11 21:23:57 +0800
...
void solve(){
    ...
    FOR(i, 0, n){
        vector<vector<int>>dp2(m+1, vector<int>(2, -INF));
        FOR(j, 0, m+1){
            if(dp[j][0]!=-INF){
                dp2[j][0] = max(dp2[j][0], dp[j][0]+arr[i]);
            }
            if(j>0 and dp[j-1][1]!=-INF){
                dp2[j][0] = max(dp2[j][0], dp[j-1][1]+arr[i]);
            }

            if(dp[j][1]!=-INF){
                dp2[j][1] = max(dp2[j][0], dp[j][1]+brr[i]);
            }
            if(j>0 and dp[j-1][0]!=-INF){
                dp2[j][1] = max(dp2[j][1], dp[j-1][0]+brr[i]);
            }
        }
        dp = dp2;
    }
