 ...
 * Platform:       AtCoder
 * Problem:        abc467_e — Adjacent Sums (hard)
 * Problem URL:    https://atcoder.jp/contests/abc467/tasks/abc467_e
 *
 * Matched result: AC
 * Submission:     #77630669
 * Submission URL: https://atcoder.jp/contests/abc467/submissions/77630669
 * Submitted:      2026-07-18 21:05:30 +0800
...
void solve(){
    ...
    FOR(i, 0, n) cin >> arr[i];
    vector<int>brr(n);
    FOR(i, 0, n-1) cin >> brr[i];

    vector<int>crr(n);
    crr[0] = 0;
    FOR(i, 1, n){
        crr[i] = (brr[i-1]-crr[i-1])%m;
        if(crr[i]<0) crr[i] += m;
    }
