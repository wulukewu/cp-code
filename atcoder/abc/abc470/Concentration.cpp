 ...
 * Platform:       AtCoder
 * Problem:        abc470_e — Concentration
 * Problem URL:    https://atcoder.jp/contests/abc470/tasks/abc470_e
 *
 * Matched result: AC
 * Submission:     #78227187
 * Submission URL: https://atcoder.jp/contests/abc470/submissions/78227187
 * Submitted:      2026-08-08 21:21:00 +0800
 * Language:       C++23 (GCC 15.2.0)
 * Runtime:        23 ms
 * Points:         450.0
 * Submissions:    1 total / 1 accepted
 * File commit:    2026-08-08 21:31:22 +0800
...
void solve(){
    ...
    int N = n+1;
    vector<vector<double>>prv(N, vector<double>(N, 0));
    ...
    FOR(l, 1, L+1){
        ...
        FOR(k, 0, N){
            FOR(m, 0, k+1){
                ...
                double um1 = u-1;
                double inner = 0;
                inner += (1.0/um1) * (1+cur[m-1][s]);
                if(s>0){
                    double bonus = 0;
                    if(l>=2) bonus = 1+prv[m-1][s];
                    inner += (s/um1)*bonus;
                }
                if(m>=2){
                    inner += (2.0*m-2)/um1 * prv[m-2][s+2];
                }
                val += 2.0*m/u*inner;
                cur[m][s] = val;
            }
    ...
    double ans = prv[n][0] * ((double)sum/n);
