...
........:::.......:::..::::..::........::
 ...
 CODEFORCES // 2237B // Annoying the Ghost
 https://codeforces.com/contest/2237/problem/B

 AC // C++23 (GCC 14-64, msys2) // 31 ms // 0 B
 #379389504 // 2026-06-18 23:30:55 +0800
 rating 1000 // brute force, greedy
 https://codeforces.com/contest/2237/submission/379389504
...
void solve(){
    ...
    // set<int>st;
    // FOR(i, 0, n){
    //     while(crr[idx].F<=brr[i] and idx<n){
    //         st.insert(idx);
    //         idx++;
    //     }
    //     if(st.empty()){
    ...
    FOR(i, 0, n) st.insert({brr[i], i});

    vector<int>perm(n);
    int idx = 0;
    FOR(i, 0, n){
        // while(idx<n and brr[idx]<arr[i].F) idx++;
        // if(idx>=n){
        //     cout << -1 << endl;
        //     return;
        // }
        // perm[arr[i].S] = idx+1;
        // idx++;
        auto it = st.lower_bound({arr[i].F, -1});
        if(it==st.end()){
            cout << -1 << endl;
            return;
        }
