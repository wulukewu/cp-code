 ...
 * Platform:       Codeforces
 * Problem:        2263C2 — Floor of MEX (Hard Version)
 * Problem URL:    https://codeforces.com/contest/2263/problem/C2
 *
 * Matched result: AC
 * Submission:     #390460609
 * Submission URL: https://codeforces.com/contest/2263/submission/390460609
 * Submitted:      2026-09-12 23:47:17 +0800
...
void solve(){
    ...
    FOR(i, 0, n){
        int before = total;
        if(need[i]>cut){
            if(need[i]>=0 and empty){
                submod(total, empty);
                empty = 0;
            }
            FOR(j, max(0ll, cut), need[i]){
                submod(total, dp[j]);
                dp[j] = 0;
            }
            cut = need[i];
