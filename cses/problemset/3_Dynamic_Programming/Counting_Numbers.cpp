...
void solve(){
    ...
    auto f = [](int x) -> int {
        ...
        vector<vector<vector<int>>>dp(n, vector<vector<int>>(11, vector<int>(2, -1)));
        auto dfs = [&](auto&& self, int pos, int prev, int tight) -> int {
            if(pos==n){
                return 1;
            }
            ...
            FOR(d, 0, up+1){
                if(prev!=10 and d==prev) continue;
                int nprev;
                if(prev==10 and d==0){
                    nprev = 10;
                }else{
                ...
                res += self(self, pos+1, nprev, ntight);
            }
            return dp[pos][prev][tight] = res;
        };
        return dfs(dfs, 0, 10, 1);
    };
