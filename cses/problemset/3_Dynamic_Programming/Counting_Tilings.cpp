...
void solve(){
    ...
    // dp[c][mask] = 前 c 欄已經處理完，且目前第 c 欄有 mask 這些位置被上一欄伸過來佔住時的方案數
    vector<vector<int>>dp(m+1, vector<int>(1<<n, 0));
    dp[0][0] = 1;

    // dfs 的功能：在固定第 c 欄、已知 mask 的情況下，從第 r 格開始往下填完這一欄，並枚舉所有可能產生的 nmask
    // mask  = 目前這一欄，哪些格子已經被「上一欄」伸過來佔住
    // nmask = 下一欄，哪些格子會被「目前這一欄」伸過去佔住
    auto dfs = [&](auto&& self, int c, int r, int mask, int nmask) -> void {
        // r == n 代表：目前這一欄已經全部填完 所以把方案轉移到下一欄
        if(r==n){
            addmod(dp[c+1][nmask], dp[c][mask]);
            return;
        }
        // mask 第 r bit = 1：代表這格已經被上一欄的橫 domino 佔住，不用再放，直接往下
        if(mask&(1<<r)){
            self(self, c, r+1, mask, nmask);
            return;
        }

        // 橫放 domino：目前這格 + 下一欄同一個 row 所以 nmask 的第 r bit 要設成 1
        self(self, c, r+1, mask, nmask | (1<<r));
        // 直放 domino：使用目前 row 和 row+1，所以要確認 row+1 存在且沒被佔
        if(r+1<n and !(mask & (1<<(r+1)))){
            self(self, c, r+2, mask, nmask);
        }
    };
    ...
    // FOR(c, 0, m) print(dp[c]);

    // dp[m][0]：m 欄全部處理完，且沒有 domino 再伸到棋盤外
    cout << dp[m][0] << endl;
}
