...
void solve(){
    ...
    vector<pair<int, int>>st; // F: 山的高度 S: 山的L
    int ans = 0;
    FOR(i, 0, n+1){
        int cur;
        if(i==n) cur = LLONG_MAX;
        else cur = h[i];
        int c = 0; // 這輪pop中的最大值
        int ph = -1, pl = 0;
        while(!st.empty() and st.back().F<cur){
            int h = st.back().F;
            int l = st.back().S;
            st.pop_back();
            int r;
        ...
        if(i<n) st.PB({cur, c});
        // for(auto&& j: st) cout << j.F << ' ';
        // cout << endl;
        // for(auto&& j: st) cout << j.S << ' ';
        // cout << endl;

        // int c = 0;
        // while(!st.empty() and st.back().F<h[i]){
        //     int d = st.back().S;
        //     d = max(d, 1+c);
        //     c = d;
        //     st.pop_back();
