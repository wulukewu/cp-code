...
void range_increment(int i, int l, int r, ll v) {
    ...
    push(i);
    range_increment(2 * i, l, r, v);
    range_increment(2 * i + 1, l, r, v);
    pull(i);
}

void range_assign(int i, int l, int r, ll v) {
    if (l > hi[i] || r < lo[i]) return;
    if (l <= lo[i] && hi[i] <= r) {
        assign(i, v);
        return;
    ...
    push(i);
    range_assign(2 * i, l, r, v);
    range_assign(2 * i + 1, l, r, v);
    pull(i);
}
...
int main() {
    ...
    for (int q = 0; q < Q; q++) {
        ...
        if (t == 1) {
            cin >> x;
            range_increment(1, a, b, x);
        } else if (t == 2) {
            cin >> x;
            range_assign(1, a, b, x);
        } else {
            cout << query(1, a, b) << '\n';
        }
    }
